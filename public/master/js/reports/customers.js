...
function loadTable(){
    table[0] = $(`#table_datatable`).DataTable({
        ajax: {
            url: base_url(['dashboard/reports/data/customers']),
            data: function(d) {
        ...
        buttons: [
            {
                ...
                filename: 'Reporte_Clientes', // 🔹 Define aquí el nombre del archivo
                title: function () {
                    ...
                    let title = !!customer ? `Reporte Cliente: ${customer.name}` : "Reporte Clientes";
                    title += !!type_document ? ` | ${type_document.name} - ${type_document.code}` : "";
                    title += ` | ${dates}`;
                    return title;
                },
                exportOptions: {
                    // columns: $('#customer_filter').val() == "" ? [0, 1, 2, 3, 5, 6, 7] : [0, 1, 2, 3],
                    format: {
                ...
                action: async function (e, dt, button, config) {
                    ...
                    const url_data = base_url(['dashboard/reports/data/customers'], getData);
        ...
        drawCallback: (setting) => {
            ...
            if(setting.json != undefined){
                ...
                table[0].column(4).visible(!customer);
